...
define h = Character("???", color="#c80000")
...
init python:   
    ...
    class GameConfig:
        def __init__(self,
                     ...
                     target_img_path='minigames/hunt/imgs/targets1/',
                     target_nb=4,
                     time_limit=10,
                     life_max=3,
                     round_nb=3,
            ...
            self.IMG_BULLET = 'minigames/hunt/imgs/bullets/bullet.png'
            self.IMG_BULLET_EMPTY = 'minigames/hunt/imgs/bullets/bullet_empty.png'
            self.IMG_AIM_IDLE = 'minigames/hunt/imgs/weapon/crosshair.png'
            self.IMG_AIM_HOVER = 'minigames/hunt/imgs/weapon/crosshair_focused.png'
            self.IMG_WEAPON = 'minigames/hunt/imgs/weapon/gun.png'
            self.IMG_HEART = 'minigames/hunt/imgs/life/heart.png'
            self.IMG_HEART_EMPTY = 'minigames/hunt/imgs/life/heart_empty.png'
...
init:
    ...
    screen board():
        ...
        frame align (1.0, 0.0):
            ...
            background "#4f5a6680"
            hbox:
                text "{b}{i}Life:{/i}{/b}  "  size 35 color "#ff5858" yalign 0.5
...
init python:
    ...
    class HuntingGame:
        ...
        def run(self):
            ...
            self.round_end("У чучела нет песни, только эхо, но оно — часть реки.", is_game_over=True)
            return None
            
        def round_init(self):
            # Initialize round elements
            self.status.round_init()
            self.round_cnt = self.status.round_now
            renpy.scene('black')
            renpy.say(who=h, what="{w=1.0}{cps=50}УБИВАТЬ{/cps}")
            renpy.show_screen("board")
            # Clear existing targets and create new ones
            self.targets.clear()
            self.targets = [self.Target(self.config, i) for i in range(self.config.target_nb)]
        ...
        def handle_events(self):
            ...
            if self.status.is_game_over():
                self.round_end("Клетки", True)
                return
            if self.status.is_clear():
                words = ["Связаны", "Клетки", "Почему чучело избегает окон?", "Чучело вызывает рябь на наших висках, наши вены почти соприкасаются, оно стесняется", "Мы наблюдаем за волнами чучел на мелководье, проходящими сквозь окна, и все они одинаковы."]
        ...
        class Target:
            def __init__(self, config, id):
                ...
                self.image_ref = Image(self.image_path)
                ...
                self.position = At(self.image_ref, moving_target(self.target_speed, self.target_ypos, self.target_scale))
                self.killed = False
